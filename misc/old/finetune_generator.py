...
def selectRandomNodes(nd_to_sample, batch_size):
    ...
    ind_fixed_nodes = torch.zeros((nd_to_sample.shape[0], 1))
    ind_fixed_nodes[fixed_nodes] = 1.0
    ind_fixed_nodes = ind_fixed_nodes.cuda()
    return ind_fixed_nodes
...
for k in range(1000):
    for i, batch in enumerate(ae_loader):
        ...
        gen_mks, curr_feats = generator(z, ind_fixed_nodes, given_masks, given_nds, given_eds, given_v=mask, state=fixed_nodes_state)
        
        # Select random nodes
        ind_fixed_nodes = selectRandomNodes(nd_to_sample, batch_size)
