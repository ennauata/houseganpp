...
from misc.utils import combine_images_maps, rectangle_renderer
from models.models_exp_high_res import Discriminator, Generator, compute_gradient_penalty, weights_init_normal
...
def selectNodesTypes(nd_to_sample, batch_size, nds):
    ...
    for k in range(batch_size):
        ...
        N = np.random.randint(0, len(set(_types))) # pick N types
        _t = np.random.choice(list(set(_types)), size=N, replace=False) # -1 is for no types selected
        fixed_rooms = [r for r, _t_x in enumerate(_types) if _t_x in _t]
        print(' selected types: {} \n fixed rooms: {}'.format('-'.join([str(i) for i in _t]), '-'.join([str(i) for i in fixed_rooms])))
        # N = np.random.randint(rooms_num, size=1)
        # select random nodes or all nodes!
        # if np.random.normal(0, 1) > 0.5:
